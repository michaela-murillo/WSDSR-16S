...
#   [x] Designated color for every 2-MIB producer cyanobacteria for consistency
...
filepath <- "C:/Users/Mikki/Documents/WSD/ONS_week1-4/serviceRun/raw_batch3_check"

# Path to dir for storing excel files
# Note: must replace \ with \\
excelOutput_path <- "C:\\Users\\Mikki\\Documents\\WSD\\ONS_week1-4\\serviceRun\\raw_batch3_check"

# Batch name
batchName <- "Batch003_R1"

# Sample collection date
samplingdate <- "20231122"
...
mib_db <- read.delim("C:/Users/Mikki/Documents/WSD/ONS_week1-4/serviceRun/mibProducer_db.txt")
...
# Filter for known 2-MIB producers
mibproducing_cyano <- data.frame()

for (i in 1:nrow(subset_cyano)) {
  if (mibProducers$mib_production[i] == "Y") {
    genera <- df2$genera[i]
    
    # Step 5: Subset the rows from df1 where genera matches the value from df2
    subset_rows <- df1[df1$genera == genera, ]
    
    # Step 6: Append the subset_rows to df3
    df3 <- rbind(df3, subset_rows)
  }
}
